...
int mc2err_load(struct mc2err_data *m2e, char *filename)
{
    ...
    {
        num_in = m2e->chain_level[i]*m2e->length*m2e->width;
    ...
    num_in = m2e->num_level*m2e->length;
    ...
    num_in = m2e->num_level*m2e->length*m2e->width;
    ...
    num_in = m2e->num_level*m2e->num_level*m2e->length*m2e->length;
    ...
    num_in = m2e->num_level*m2e->num_level*m2e->length*m2e->length*m2e->width*(m2e->width+1)/2;
    ...
    num_in = m2e->num_level*(m2e->num_level-1)*m2e->width*(m2e->width+1)/2;
    ...
    // handle remaining errors & return
    return 0;
}
